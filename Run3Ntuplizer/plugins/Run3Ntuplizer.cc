...
//      edm::EDGetTokenT<pat::PackedTriggerPrescales> triggerPrescalesToken_;
      ...
      std::vector<Float_t>  Jet_En_;
      std::vector<Float_t>  Jet_eta_;
      std::vector<Float_t>  Jet_phi_;
      std::vector<Float_t>  Jet_neEmEF_; //jet neutral EM energy fraction
      std::vector<Float_t>  Jet_chEmEF_; //jet charged EM energy fraction
      std::vector<Float_t>  Jet_neHadEF_; //jet neutral Had energy fraction
      std::vector<Float_t>  Jet_chHadEF_; //jet charged Had energy fraction
      std::vector<Float_t>  Jet_dPhi_;   //dphi (jet , MET)
      Bool_t isBadHotEcalTower_;
  ...
  effectiveAreas_((iConfig.getParameter<edm::FileInPath>("effAreasConfigFile")).fullPath())
{
     ...
     jetToken_ = consumes<View<pat::Jet>>(iConfig.getParameter<InputTag>("puppijets"));
...
//     triggerPrescalesToken_ = consumes<pat::PackedTriggerPrescales>(iConfig.getParameter<edm::InputTag>("triggerPrescales")); 
     ...
     tree_->Branch("BAD_run"                     , &BAD_run_   , "run/I"  );
     tree_->Branch("BAD_lumis"                   , &BAD_lumis_ , "lumis/I");
     tree_->Branch("BAD_event"                   , &BAD_event_ );
     ...
     tree_->Branch("nJet_highMET"              ,  &nJet_highMET_);
     tree_->Branch("Jet_pt"            ,  &Jet_pt_);
     tree_->Branch("Jet_En"            ,  &Jet_En_);
     tree_->Branch("Jet_eta"           ,  &Jet_eta_);
     tree_->Branch("Jet_phi"           ,  &Jet_phi_);
     tree_->Branch("Jet_neEmEF"        ,  &Jet_neEmEF_); 
     tree_->Branch("Jet_chEmEF"        ,  &Jet_chEmEF_); 
     tree_->Branch("Jet_neHadEF"       ,  &Jet_neHadEF_); 
     tree_->Branch("Jet_chHadEF"       ,  &Jet_chHadEF_); 
     tree_->Branch("Jet_dPhi"          ,  &Jet_dPhi_);   
     tree_->Branch("isBadHotEcalTower" ,  &isBadHotEcalTower_);
...
*/
  ...
  Jet_chHadEF_.clear(); 
  Jet_dPhi_.clear();   
  isBadHotEcalTower_ = 0;

  edm::Handle<edm::View<pat::Jet> > jetHandle;
  iEvent.getByToken(jetToken_, jetHandle);

  double jet_pt  = -999 ; //used for trg matching at the last part
  double jet_eta = -999 ;
  double jet_phi = -999 ;
  double jet_chEmEF = -999 ;
  double jet_neEmEF = -999 ;
  int isBadHotEcalTower = 0;

  if (!jetHandle.isValid()) {printf("no pat::Jets (AK4) in event");}
  for (size_t i = 0; i < jetHandle->size(); ++i){
  //for (edm::View<pat::Jet>::const_iterator iJet = jetHandle->begin(); iJet != jetHandle->end(); ++iJet) {

    jet_pt  = jetHandle->ptrAt(0)->pt();
    jet_eta = jetHandle->ptrAt(0)->eta() ;
    jet_phi = jetHandle->ptrAt(0)->phi() ;
    jet_chEmEF = jetHandle->ptrAt(0)->chargedEmEnergyFraction();
    jet_neEmEF = jetHandle->ptrAt(0)->neutralEmEnergyFraction();

    const auto iJet = jetHandle->ptrAt(i);
    if (iJet->pt() < 50) continue;
    Jet_pt_.push_back( iJet->pt());
    Jet_En_.push_back( iJet->energy());
    Jet_eta_.push_back(iJet->eta());
    Jet_phi_.push_back(iJet->phi());
    Jet_chEmEF_.push_back( iJet->chargedEmEnergyFraction());
    Jet_neEmEF_.push_back( iJet->neutralEmEnergyFraction());
    Jet_chHadEF_.push_back( iJet->chargedHadronEnergyFraction());
    Jet_neHadEF_.push_back( iJet->neutralHadronEnergyFraction());
    Jet_dPhi_.push_back( deltaPhi(jet_phi, puppiMET_Phi_) ) ;

    if ( puppiMET_ > 100 and jet_pt > 50 and 
         (jet_eta < -0.1 and jet_eta > -0.5) and
         (jet_phi < -1.8 and jet_phi > -2.1) and
         (jet_chEmEF > 0.9 or jet_neEmEF > 0.9) and
         deltaPhi(jet_phi, puppiMET_Phi_) > 2.9 ) {
        isBadHotEcalTower = 1;
        BAD_run_    = iEvent.id().run();
        BAD_event_  = iEvent.id().event();
        BAD_lumis_  = iEvent.luminosityBlock();
        cout << "@@@ Found isBadHotEcalTower" << isBadHotEcalTower << " Run:Lumi:Event = " << BAD_run_ << ":" << BAD_lumis_ << ":" << BAD_event_ << endl;
        cout << "MET > 100: " << puppiMET_ << ", jetpt>50: " << jet_pt << ", -0.5<jet_eta<-0.1: " << jet_eta << ", -2.1<jet_phi<-1.8 " << jet_phi << ", jet_chEmEF>0.8: " << jet_chEmEF << ", jet_neEmEF>0.9: " << jet_neEmEF << ", dPhi(jet,met)>2.9: " << deltaPhi(jet_phi, puppiMET_Phi_) << endl;

    nJet_highMET_++;
    } 
    isBadHotEcalTower_ = isBadHotEcalTower;
