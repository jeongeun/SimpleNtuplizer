...
def add_cms_lumi(pad, lumi_text="9.5 fb^{-1} (13.6 TeV)", cms_text="CMS", extra_text="Preliminary"):
    pad.cd()
    latex = ROOT.TLatex()
    latex.SetNDC()
...
def draw1D():
    ...
    # Open data files
    DATA = "heff_23Muon_ReReco_D.root"
    MC = "heff_23MC_postBPIX.root"
    ...
    leg_eb.AddEntry(eff_22FG_hltORALL_eb_pt, "Data (Run2022CD 22Sep2023)", "pl")
    leg_eb.AddEntry(eff_22MC_hltORALL_eb_pt, "MC (t#bar{t}#rightarrow2l2#nu preEE 130X)", "f")
    leg_eb.Draw()

    add_cms_lumi(padU, lumi_text="9.5 fb^{-1} (13.6 TeV)", cms_text="CMS", extra_text="Preliminary")
    ...
    initial_value = calculate_median_value(hSF_hltORALL_eb_pt, 131., 3000.)  # Calculate initial value for the fit
    print("Initial value for fit:", initial_value)
    constant_fit_eb = TF1("constant_fit_eb", "[0]", 131, 3000)  # Define a constant function
    constant_fit_eb.SetParameter(0, initial_value)  # Set initial parameter value
    constant_fit_eb.SetParLimits(0, initial_value+0.01, 1.0001) 
    ...
    Err_eb = math.sqrt((errConst_eb * errConst_eb) + (0.0011 * 0.0011))
    ...
    canvas_eb.SaveAs("ceb_1sfpt_23postBPIX_sf_fit.png")
...
########################################################
    ...
    leg_ee.AddEntry(eff_22FG_hltORALL_ee_pt, "Data (Run2023D 22Sep2023)", "pl")
    leg_ee.AddEntry(eff_22MC_hltORALL_ee_pt, "MC (t#bar{t}#rightarrow2l2#nu postBPIX 130X)", "f")
    leg_ee.Draw()

    add_cms_lumi(padU1, lumi_text="9.5 fb^{-1} (13.6 TeV)", cms_text="CMS", extra_text="Preliminary")
    ...
    initial_value = calculate_median_value(hSF_hltORALL_ee_pt, 131., 3000.)  # Calculate initial value for the fit
    print("Initial value for fit:", initial_value)
    constant_fit_ee = TF1("constant_fit_ee", "[0]", 131, 3000)  # Define a constant function
    constant_fit_ee.SetParameter(0, initial_value)  # Set initial parameter value
    constant_fit_ee.SetParLimits(0, initial_value+0.005, 1.001) 
    ...
    Err_ee = math.sqrt((errConst_ee * errConst_ee) + (0.0003 * 0.0003))
    ...
    canvas_ee.SaveAs("cee_1sfpt_23postBPIX_sf_fit.png")
